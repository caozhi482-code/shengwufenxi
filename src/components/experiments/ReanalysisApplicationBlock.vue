    ...
    <table class="w-full min-w-[1000px] border-collapse table-fixed text-[13px] text-[--text-main]">
      ...
      <tbody>
        ...
        <tr v-for="(row, i) in model.rows" :key="row.id" class="h-[42px]">
          <td class="ra-cell bg-[#f3efe0]"><CellEditor :value="row.sampleId" :editable="editable" @update="updateRow(i, 'sampleId', $event)" /></td>
          <td class="ra-cell"><CellEditor :value="row.initialResult" :editable="editable" @update="updateRow(i, 'initialResult', $event)" /></td>
          <td class="ra-cell"><CellEditor :value="row.initialBatch" :editable="editable" @update="updateRow(i, 'initialBatch', $event)" /></td>
          <td class="ra-cell"><CellEditor :value="row.reason" :editable="editable" @update="updateRow(i, 'reason', $event)" /></td>
          <td class="ra-cell"><CellEditor :value="String(row.count)" :editable="editable" type="number" @update="updateRow(i, 'count', Number($event) || 0)" /></td>
          <td class="ra-cell"><CellEditor :value="row.reBatch" :editable="editable" @update="updateRow(i, 'reBatch', $event)" /></td>
        </tr>
