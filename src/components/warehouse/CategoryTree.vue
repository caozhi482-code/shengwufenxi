  ...
  <div class="space-y-3">
    <div v-if="node.children?.length" class="tree-node">
      ...
      >
        ...
        <span>{{ node.name }}</span>
        <span v-if="node.itemCount" class="text-xs text-[--muted-foreground] ml-auto">({{ node.itemCount }})</span>
      </div>
...
defineProps<{ node: Category & { selected?: boolean; expanded?: boolean; children?: (Category & { selected?: boolean; expanded?: boolean })[] } }>();
defineEmits<{ select: [node: Category & { selected?: boolean; expanded?: boolean }] }>();
</script>
