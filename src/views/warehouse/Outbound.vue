<template>
  <div class="space-y-5">
    <BasePageHeader title="出库申请" subtitle="创建和管理出库申请单" />
    ...
    <BaseCard v-if="['审批中', '已通过', '已拒绝'].includes(currentForm.status)">
      ...
      <div class="space-y-3">
        <div class="flex items-start gap-3 py-2 border-b border-[--border]">
          <div class="w-8 h-8 rounded-full bg-[--primary-soft] text-[--primary] flex items-center justify-center text-xs font-bold shrink-0">李</div>
          <div>
            <div class="text-sm font-medium text-[--text-main]">李四 提交申请</div>
            <div class="text-xs text-[--muted-foreground]">{{ currentForm.createdAt }} 10:20</div>
          </div>
          <BaseTag tone="warning" label="待审批" />
        </div>
      </div>
    </BaseCard>
  </div>
...
import { ref } from 'vue';
import { outboundApplications } from '@/api/mock/warehouse';
import type { OutboundApplication } from '@/api/mock/warehouse';
import BasePageHeader from '@/components/base/BasePageHeader.vue';
import BaseCard from '@/components/base/BaseCard.vue';
import BaseButton from '@/components/base/BaseButton.vue';
import BaseTag from '@/components/base/BaseTag.vue';
import BaseFormField from '@/components/base/BaseFormField.vue';
import OutboundForm from '@/components/warehouse/OutboundForm.vue';

const currentForm = ref<OutboundApplication>(outboundApplications[0]);
